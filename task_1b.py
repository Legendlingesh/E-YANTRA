...
def transform_vision_sensor_image(vision_sensor_image, image_resolution):
	...
	##############	ADD YOUR CODE HERE	##############
	vision_sensor_image = np.array(vision_sensor_image)
	vision_sensor_image = vision_sensor_image.astype('uint8')
	image = vision_sensor_image.reshape((image_resolution[0],image_resolution[1],3))


    # converting BGR to RGB
	image_trans= cv2.cvtColor(image , cv2.COLOR_BGR2RGB)

	transformed_image = cv2.flip(image_trans, 0)   
...
def detect_qr_codes(transformed_image):
	...
	for barcode in barcodes: 
		...
		qr_codes.append([barcode_info, centroid])
	return qr_codes
